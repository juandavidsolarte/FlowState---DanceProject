...
class RegistrationFlowTests(TestCase):
    ...
    def test_duplicate_email_registration_fails(self, mock_send):
        User.objects.create_user(email='ana@example.com', password='StrongPass123', first_name='Ana', last_name='García')
        response = self.client.post(
            reverse('users:register'),
            data=json.dumps(self.payload),
            content_type='application/json',
    ...
    def test_underage_registration_fails(self, mock_send):
        payload = {**self.payload, 'email': 'minor@example.com', 'date_of_birth': '2010-01-01'}
        response = self.client.post(
            reverse('users:register'),
...
class VerificationFlowTests(TestCase):
    ...
    def test_email_verification_success(self):
        response = self.client.get(reverse('users:verify-email', kwargs={'token': self.user.verification_token}))
    ...
    def test_invalid_token_fails(self):
        response = self.client.get(reverse('users:verify-email', kwargs={'token': uuid4()}))
        self.assertEqual(response.status_code, 400)
